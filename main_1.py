...
def run_ui(ui_type: str, best_route: Optional[Route] = None, desktop_window=None) -> None:
    """
    ...
    """
    if ui_type == "desktop":
        visualize_desktop_ui(best_route, True, desktop_window)
    elif ui_type == "web":
...
def main():
    ...
    try:
        ...
        progress_tracker = ProgressTracker(args.ui, total_iterations=1e3)

        # Initialize and run selected algorithm
        algorithm = get_algorithm(args.algorithm, progress_tracker)
        best_route, desktop_window = algorithm.optimize(initial_route)
        ...
        run_ui(args.ui, best_route, desktop_window)
